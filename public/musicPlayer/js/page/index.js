require(["/musicPlayer/js/config.js"], function() {
  require(["jquery", "template", "dragdrop", "scrollbar"], function(
    $,
    template,
    dragdrop,
    scrollbar
    ...
    var musicPlayer = {
      ...
      aidTime: function(element, box) {
        var im = isNaN(parseInt(element.currentTime / 60))
          ? "00"
          : parseInt(element.currentTime / 60);
        var ic = isNaN(parseInt(element.currentTime % 60))
          ? "00"
          : parseInt(element.currentTime % 60);
        var zm = isNaN(parseInt(element.duration / 60))
          ? "00"
          : parseInt(element.duration / 60);
        var zc = isNaN(parseInt(element.duration % 60))
          ? "00"
          : parseInt(element.duration % 60);
    ...
    var lyrictObj = {
      ...
      getLrc: function() {
        //判断是否切换了新歌曲
        if (this.lrcid != musicPlayer.now) {
          $("#lyric")
            .html("")
            .css("top", 300);
          lyrictObj.past = 0;
          if (this.lyrict) clearInterval(this.lyrict);
          var lyricObjArr = this.lyricObjArr;
          var lyricObjArr = [];
          $.get(
            "/lyric",
            {
            ...
            function(data) {
              ...
              var lyricArr = data.lrc.lyric.split("\n");
              lyricArr.forEach(function(value, index) {
                var time = value.match(/^\[.+?\]/);
                if (time) {
                  time = time[0].slice(1, 6).split(":");
              ...
              lyrictObj.lyricObjArr = [].concat(lyricObjArr);
              if (lyricObjArr.length > 0) {
                lyricObjArr.push({
                  time: 10000,
                  str: "",
                  active: false
                });
                $("#lyric").html(template("lyrictem", lyricObjArr));
                var n = lyrictObj.changeTime(lyricObjArr);
                $("#lyric").css({
                  top: 300 - n * 30
                });
              }
              lyrictObj.showLrc(lyricObjArr);
            }
          );
        } else {
          lyrictObj.showLrc(this.lyricObjArr);
        }
      },
      showLrc: function(lyricObjArr) {
        if (this.lrcid != musicPlayer.now) {
          //重新记录歌曲ID
          this.lrcid = musicPlayer.now;
          if (lyricObjArr.length > 0) {
            $("#masklyric").css("display", "flex");
            this.lyrict = setInterval(function() {
              if (lyricObjArr && lyricObjArr.length > 0) {
                for (var i = lyrictObj.past; i < lyricObjArr.length - 1; i++) {
                  var nowtime = $("#audio").get(0).currentTime;
                  if (
                    parseInt(nowtime) >= lyricObjArr[i].time &&
                    parseInt(nowtime) < lyricObjArr[i + 1].time &&
                    lyricObjArr[i].active == false
                  ) {
                    if (lyricObjArr[i - 1]) {
                      lyricObjArr[i - 1].active = false;
                    }
                    lyricObjArr[i].active = true;
                    $("#lyric>p").css("color", "#eee");
                    $("#lyric>p")
                      .eq(i)
                      .css("color", "#bc24b7");
                    $("#lyric").animate(
      ...
      changeTime: function(lyricObjArr) {
        var nowtime = parseInt($("#audio").get(0).currentTime);
    ...
    $(function() {
      ...
      (function() {
        ...
        $("#drag").on("mousedown", function() {
          $(document).on("mousemove", function() {
            var n =
              parseInt($("#drag").position().top) /
              parseInt($("#volbar").height());
            musicPlayer.volume(
              $("#audio").get(0),
              parseInt((1 - n) * 100) / 100
            );
            ...
            $("#volbar").css({
              background:
                "linear-gradient(to top,#bc24b7 0%, #bc24b7 " +
                parseInt((1 - n) * 100) +
                "%, #999 " +
                parseInt((1 - n) * 100) +
                "%, #999 100%)"
      ...
      (function() {
        ...
        $("#vdrag").on("mousedown", function() {
          $(document).on("mousemove", function() {
            var n =
              parseInt($("#vdrag").position().top) /
              parseInt($(".vvolumebar").height());
            videoPlayer.volume(
              $("#video").get(0),
              parseInt((1 - n) * 100) / 100
            );
      ...
      (function() {
        ...
        $("#player").on("click", ".refresh", function() {
          ...
          //更改字幕位置
          if (lyrictObj.lyricObjArr.length > 0) {
            var n = lyrictObj.changeTime(lyrictObj.lyricObjArr);
            $("#lyric").css({
              top: 300 - n * 30
            });
            lyrictObj.past = n;
        ...
        $("#player").on("click", ".playPgbar", function(e) {
          var left =
            parseInt(e.pageX) - parseInt($(".playPgbar").offset().left);
          var currentTime = parseInt(
            $("#audio").get(0).duration *
              (left / parseInt($(".playPgbar").width()))
          );
        ...
        //歌词获取与显示
        $("#player").on("click", ".lyric", function() {
          lyrictObj.getLrc();
        });
          ...
          switch ($(this).index()) {
            case 0: {
              $.get(
                ...
                function(data) {
                  var data = JSON.parse(data);
                  $(".sbmright>ul")
                    .html(template("menuright", data.artists))
                    .attr("data-kind", "artists");
                  //左侧菜单自定义滚动条
            ...
            case 1: {
              $.get(
                ...
                function(data) {
                  var data = JSON.parse(data);
                  $(".sbmright>ul")
                    .html(template("menuright", data.playlists))
                    .attr("data-kind", "playlist");
                  //左侧菜单自定义滚动条
                  scrollbar("sbmright", "scrollbar");
                }
              );
            ...
            case 2: {
              $.get(
                ...
                function(data) {
                  ...
                  $(".sbmright>ul")
                    .html(template("menuright", data.result.tracks))
                    .attr("data-kind", "musiclist");
                  //左侧菜单自定义滚动条
                  scrollbar("sbmright", "scrollbar");
                }
              );
            ...
            case 5: {
              $.get(
                ...
                function(data) {
                  $(".sbmright>ul")
                    .html(template("menuright", data.data))
                    .attr("data-kind", "mvlist");
                  //左侧菜单自定义滚动条
                  scrollbar("sbmright", "scrollbar");
                }
              );
              ...
              $.get(
                ...
                function(data) {
                  ...
                  if (data.code == 200) {
                    ...
                    $(".mrcontent>ul").html(
                      template("searchtem", data.result.songs)
                    );
                    $(".maskresult")
                      .fadeIn(600)
                      .css("display", "flex");
              ...
              $.get(
                ...
                function(data) {
                  ...
                  if (data.code == 200) {
                    ...
                    $(".mrcontent>ul").html(
                      template("playlisttem", data.playlist.tracks)
                    );
                    $(".maskresult")
                      .fadeIn(600)
                      .css("display", "flex");
        ...
        $("#vcontrols").on("click", ".full", function() {
          if (
            parseInt($("#videobox").css("width")) != parseInt($(window).width())
          ) {
